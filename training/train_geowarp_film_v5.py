"""
GeoWarpFiLMNet v5 訓練腳本
v5 改進（相對 v4）：
- 加入 ITDConsistencyLoss：防止 phase residual 抹掉 GeometricWarper 建立的 ITD
- 問題背景：v4 的 model ITD ≈ 0（phase residual 把 ITD 破壞），導致靜態角度誤差大
- Stage 1：mag_anchor + phase + ITD consistency（讓 ITD 從一開始就被保護）
- Stage 2：L2 + Phase + IPD + ITD consistency
"""
import sys
...
from src.losses import L2Loss, PhaseLoss, ITDConsistencyLoss
...
config = {
    ...
    'output_dir':     'geowarp_film_v5b',
    'checkpoint':     'geowarp_film_v5b/best.net',
    'log_file':       'geowarp_film_v5b/train.log',
    ...
    # ITD loss weight（小權重，避免壓制 phase 學習）
    'w_itd':          0.5,
}
