"""
GeoWarpFiLMNet 訓練腳本
Stage 1: IPD loss（讓 phase 先收斂）
Stage 2: L2 + PhaseLoss + IPD（全局 fine-tune）
"""
import sys
...
config = {
    ...
    'output_dir':     'geowarp_film',
    'checkpoint':     'geowarp_film/best.net',
    'log_file':       'geowarp_film/train.log',

    'stage1_epochs':  30,   # 回到 v1，避免 IPD 過擬合
    'stage2_epochs':  80,   # 70 → 80，給更多優化空間
    'lr':             1e-4,
    'batch_size':     16,
    'patience':       20,   # 保持 20，更有耐心
}
...
def run_epoch(model, loader, optimizer, device, stage, w):
    ...
    for mono, binaural, view in loader:
        ...
        if stage == 1:
            ...
            loss = ipd + 0.1 * mag_anchor + 0.1 * phase
