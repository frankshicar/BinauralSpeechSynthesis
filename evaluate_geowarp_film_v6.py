...
for subject_dir in subject_dirs:
    ...
    if sr != 48000:
        ...
        ratio = 48000 / sr
        mono_t_tmp = torch.from_numpy(mono_np)  # (1, T)
        new_len = int(mono_np.shape[1] * ratio)
        mono_np = F_nn.interpolate(mono_t_tmp.unsqueeze(0), size=new_len, mode='linear', align_corners=False).squeeze(0).numpy()
        sr = 48000
    original_mono_len = mono_np.shape[1]
    ...
    # Trim to original mono length (remove padding), matching evaluate.py behavior
    pred_L = pred_L[:original_mono_len]
    pred_R = pred_R[:original_mono_len]
