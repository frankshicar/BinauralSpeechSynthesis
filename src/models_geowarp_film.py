...
class FourierPositionEncoder(nn.Module):
    """
    Multi-scale Fourier Features for position/orientation encoding.
    Input:  view (B, 7, K)
    ...
    def forward(self, view):
        ...
        return self.mlp(feat)                           # (B, output_dim)
...
class GeoWarpFiLMNet(nn.Module):
    """
    GeometricWarper + Fourier Encoder + 64-band FiLM ResStack
    """
    def __init__(self,
                 n_fft=1024,
                 hop_length=256,
    ...
    def forward(self, mono, view):
        ...
        # 7. Output phase residual (constrain to ±π/2 to preserve geometric prior)
        phase_res = self.phase_head(x) * (torch.pi / 2)   # (B, 2, F, T_stft), scale to [-π/2, π/2]
