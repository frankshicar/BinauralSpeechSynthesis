...
v6 改進（相對 v5b）：
1. NeuralWarpCorrector：仿照 Meta Warpnet，學習修正 geometric warpfield 的 ITD 低估
   warpfield = geometric + neural（時域修正，比 STFT 域 loss 更根本）
2. TemporalPositionEncoder：per-frame conditioning，保留時間維度
   v5b 的 FourierPositionEncoder 把整段 view 壓成一個向量，動態場景每幀用同樣 conditioning
   v6 輸出 (B, pos_dim, T_stft)，每個 STFT frame 有獨立的 γ/β
3. FiLMLayer 改為接受 per-frame conditioning (B, pos_dim, T)
"""
import sys
